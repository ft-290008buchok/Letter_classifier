...
# Использование
Если вы работаете с Visual Studio, для запуска проекта требуется только клонировать данный репозиторий и выполнить сборку, графическая библиотека SFML Graphics, которая используется в проекте для рукописного ввода символов включена в проект, и подключать её не требуется.
